...
public class CalculadorDeZonas implements ITablero{
	...
	public class Zona implements ITablero{
		...
		private int _objetivosConBloqueOEnInterior = -1;
		...
		public int objetivosConBloqueOEnInterior(){
			if( _objetivosConBloqueOEnInterior != -1 ){
				return _objetivosConBloqueOEnInterior;
			}
			_objetivosConBloqueOEnInterior = 0;
			for( Punto p: interior() ){
				Objeto t = _tablero.getObjeto(p);
				if( t.hayObjetivo() ){
					_objetivosConBloqueOEnInterior++;
				}
			}
			
			for( Punto p: borde() ){
				Objeto t = _tablero.getObjeto(p);
				if( t == Objeto.BLOQUEOBJETIVO ){
					_objetivosConBloqueOEnInterior++;
				}
			}
			return _objetivosConBloqueOEnInterior;
		...
		public boolean solapaCon(Zona continente) {
			for( Punto p: interior() ){
				if( continente.enInterior(p) ){
					return true;
				}
