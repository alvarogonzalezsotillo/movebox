...
public class CalculadorDeZonasEvolucionables {
	...
	private static final PrintStream DEBUG = Movebox.NULLDEBUG;
...
//		t.dump(DEBUG);
		...
		for( Zona zona: cz.zonas() ){
			// SE HACE UNA LISTA DE ZONAS, Y SE VAN SACANDO LAS EVOLUCIONABLES
			// DESDE LAS POSICIONES ACCESIBLES DESDE EL INTERIOR DE OTRAS EVOLUCIONABLES
			// (LA PRIMERA ES LA QUE TIENE EL PERSONAJE)
			// EN LAS LLAMADAS RECURSIVAS, SE PUEDEN USAR LOS PUNTOS INICIALES Y LOS
			// QUE SE CREAN AL MOVER EL PERSONAJE
	...
	private static boolean calculaEvolucionable(Zona zona) {
		...
		boolean ret = calculaEvolucionableImpl(list, zona);
		millis = System.currentTimeMillis() - millis;
		if( millis > MAXMILLIS ){
			System.out.println( "TABLERO LENTO:" + millis);
...
//			DEBUG.println( "  " + zona.id() + " tiene el personaje" );
		...
		// SI TIENE TODOS LOS BLOQUES EN OBJETIVOS, Y NO HAY OBJETIVOS DENTRO SIN CUBRIR,
		// NO HACE FALTA ABRIR ESTA ZONA
		if( zona.objetivos() == zona.bloques()  ){
...
//		DEBUG.println( "  zona " + zona.id() + ": probare con personaje en puntos " + Arrays.asList(puntosDePrueba) );
		...
		for( PuntoYDireccion pd: puntosDePrueba ){
			...
			if( zonas.length == 0 ){
				// SI LA ZONA HA DESAPARECIDO, HAY QUE VER SI EL BLOQUE ESTA EN UN
				// OBJETIVO, EN ESE CASO LA ZONA SE HA ABIERTO
				Punto[] interior = zona.interior();
				if( interior.length > 1 ){
					throw new IllegalStateException( "Si la zona ha desaparecido tenia solo un punto interior" );
				}
				if( zona.tablero().getObjeto(interior[0]).hayObjetivo() ){
//					DEBUG.println( "  " + zona.id() + " la zona ha desaparecido dejando un bloque colocado en un objetivo" );
					return true;
				}
				if( hijo.esNoExplotableSimple() ){
//					DEBUG.println( "  " + zona.id() + " la zona ha desaparecido dejando un bloque colocado en una esquina" );
					continue;
				}
			}
			int zonasAbiertas = 0;
...
//		DEBUG.println( "  " + zona.id() + " no se pueden abrir las zonas" );
	...
	private static Zona[] zonasSolapadasConZona( Zona continente, ITablero t ){
		...
		Zona[] zonas = cz.zonas();
		for( Zona z: zonas ){
			if( z.solapaBordeConBorde(continente) ){
				ret.add(z);
			}
		}
